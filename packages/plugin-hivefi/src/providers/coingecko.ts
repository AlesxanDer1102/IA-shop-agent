...
async function fetchMarketData(
    ...
    try {
        ...
        if (axios.isAxiosError(error) && error.response?.status === 429) {
            ...
            throw new Error(
                "Rate limited by CoinGecko API and no cached data available"
            );
        }
...
export const coinGeckoProvider: Provider = {
    async get(
        ...
        try {
            ...
            const requestedTokens = message.content?.text
                ?.toLowerCase()
                .match(/\b(btc|eth|usdt|mnt|bnb|op|arb|matic|base|agni)\b/g);

            if (requestedTokens?.length) {
                selectedTokens = requestedTokens
                    .map((symbol) =>
                        TRACKED_TOKENS.find(
                            (t) => t.symbol.toLowerCase() === symbol
                        )
                    )
            ...
            const priceLines = selectedTokens
                .map((token) => {
                    ...
                    return `• ${
                        ...
                        data.usd
                    )} ${formatPriceChange(data.usd_24h_change)}`;
                })
                .filter(Boolean);
