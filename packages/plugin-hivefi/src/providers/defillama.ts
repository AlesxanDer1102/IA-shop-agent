...
async function fetchTVLData(): Promise<TVLMetrics> {
    ...
    try {
        ...
        const totalTvl = chainResponse.data.reduce(
        ...
        const actualMantleTvl = mantleProtocols.reduce(
            (sum, p) => sum + p.tvl,
            0
        );
    ...
    async get(
        ...
        try {
            ...
            return [
                ...
                `• Market Share: ${formatPercentage(
                    data.globalMetrics.mantlePercentage
                )}`,
                "",
                "Top Mantle Protocols:",
                ...data.protocols.map(
                    (p, i) => `${i + 1}. ${p.name}: ${formatTVL(p.tvl)}`
                ),
                "",
                ...
                "",
                "Top Chains by TVL:",
                ...data.globalMetrics.topChains.map(
                    (chain, i) =>
                        `${i + 1}. ${chain.name}: ${formatTVL(
                            chain.tvl
