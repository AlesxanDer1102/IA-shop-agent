    ...
    handler: async (runtime, message: Memory, state, options, callback) => {
        try {
            ...
            if (suppliedBalance < amountInWei) {
                callback?.({
                    text: `Insufficient ${tokenSymbol} supplied. You have ${formatTokenAmount(
                        suppliedBalance,
                        tokenSymbol
                    )} ${tokenSymbol} supplied, but trying to withdraw ${amount} ${tokenSymbol}`,
                });
                return false;
            }
            ...
            try {
                ...
                const newSuppliedBalance = await getATokenBalance(
                    publicClient,
                    tokenSymbol,
                    userAddress
                );
