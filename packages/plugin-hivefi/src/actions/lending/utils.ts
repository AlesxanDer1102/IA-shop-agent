...
export function parseAmountAndToken(text: string): {
    amount: string;
    tokenSymbol: keyof typeof MARKET_TOKENS | undefined;
} {
    const words = text.split(" ");
    const amount = words.find((w) => !Number.isNaN(Number(w))) || "0";
    const tokenSymbol = words
        .find((w) => Object.keys(MARKET_TOKENS).includes(w.toUpperCase()))
        ?.toUpperCase() as keyof typeof MARKET_TOKENS | undefined;
...
export function getTokenConfig(
    symbol: string
): TokenConfig & { type: "erc20"; address: Address } {
    const token = TOKENS[symbol as keyof typeof TOKENS];
    if (!token) throw new Error(`Token ${symbol} not found`);
    if (token.type !== "erc20")
        throw new Error(`Token ${symbol} is not an ERC20 token`);
    return token;
}
...
export function formatUserAccountData(data: {
    ...
    return {
        ...
        currentLiquidationThreshold:
            Number(data.currentLiquidationThreshold) / 100,
        ltv: Number(data.ltv) / 100,
        healthFactor: formatUnits(data.healthFactor, 18),
        walletBalance: data.walletBalance
            ? formatUnits(data.walletBalance, 6)
            : undefined,
        suppliedBalance: data.suppliedBalance
            ? formatUnits(data.suppliedBalance, 6)
            : undefined,
        borrowedBalance: data.borrowedBalance
            ? formatUnits(data.borrowedBalance, 6)
            : undefined,
    };
}
...
export function formatSuccessMessage(
    ...
    return [
        ...
        "",
        "📊 Position Summary",
        `💰 Total Collateral: ${Number(accountData.totalCollateralETH).toFixed(
            4
        )} USD`,
        `💳 Total Debt: ${Number(accountData.totalDebtETH).toFixed(4)} USD`,
        "",
        "📈 Account Status",
        `💵 Available to borrow: ${Number(
...
export function canBorrow(
    ...
    if (borrowAmount > availableToBorrow) {
        return {
            canBorrow: false,
            reason: `You can only borrow up to ${availableToBorrow} USD worth of ${tokenSymbol}`,
        };
    }
    ...
    if (healthFactor < 1.05) {
        return {
            canBorrow: false,
            reason: "Health factor too low. Please deposit more collateral first.",
        };
    }
...
export function formatBorrowRepayMessage(
    ...
    return [
        `Successfully ${action}ed ${amount} ${tokenSymbol} ${
            action === "borrow" ? "from" : "to"
        } Lendle`,
        "",
        `[View on Explorer](https://explorer.mantle.xyz/tx/${txHash})`,
        "",
