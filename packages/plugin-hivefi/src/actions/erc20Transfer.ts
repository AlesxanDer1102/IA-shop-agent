...
import { parseUnits, type SendTransactionParameters } from "viem";
import { mantleTestnetChain } from "../config/chains";
    ...
    handler: async (runtime, message: Memory, state, options, callback) => {
        ...
        try {
            ...
            if (!content) {
                callback?.({
                    text:
                        "Could not parse transfer details. Please use one of these formats:\n" +
            ...
            const amountHex = parseUnits(amount, token.decimals)
                .toString(16)
                .padStart(64, "0");
            ...
            callback?.({
                text: `Failed to send ${amount} ${tokenSymbol} to ${to}: ${
                    error instanceof Error ? error.message : "Unknown error"
                }. Please ensure your wallet has sufficient balance and is properly configured.`,
                content: {
                    error:
